...
function getstud() {
	...
	$.ajax({
		...
		beforeSend: function() {
			var studContent = "";
			studContent += "<tr>";
			studContent += "<td colspan=\"2\" class=\"loading\">Loading...</td>";
			studContent += "</tr>";
			$("#stud").html(studContent);
		},
		error: function() {
			var studContent = "";
			studContent += "<tr><td colspan=\"2\" class=\"error\">Oops! Failed to load students</td></tr>";
			$("#stud").html(studContent);
		},
		success: function(retData) {
			...
			if (res === "success") {
				...
				if (studJson['student'].length === 0)
					studContent += "<tr><td colspan=\"2\" class=\"loading\">There are no students</td></tr>";
				else {
					for (var i = 0; i < studJson['student'].length; i++) {
						studContent += "<tr><td>" + studJson['student'][i] + "</td><td>";
				...
				studContent += "<tr><td colspan=\"2\" class=\"error\">" + studJson + "</td></tr>";
			}
...
function fillUpdPopup(e) {
	...
	txtschNo.val(studJson["student"][i]);
	//txtInvst.val(studJson["student"][i][1]);
	txtOldschNo.val(studJson["student"][i]);
}
